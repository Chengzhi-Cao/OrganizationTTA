...
class VPTTA:
    ...
    def run(self):
        # print('self.source_dataset=',self.source_dataset)
        ...
        for batch, data in enumerate(self.target_test_loader):
            ...
            # Initialize Prompt
            if len(self.memory_bank.memory.keys()) >= self.neighbor:    # self.neighbor=16
            ...
            for tr_iter in range(self.iters):   # self.iters=1
                prompt_x, _ = self.prompt(x)    # x=[1,3,512,512], prompt_x=[1,3,512,512]
                self.model(prompt_x)
                times, bn_loss = 0, 0
