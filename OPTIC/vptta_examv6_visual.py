...
from utils.prompt_myv6_visual import Promptv6 as Prompt
...
from networks.ResUnet_TTA_visual import ResUnet
...
class VPTTA:
    ...
    def build_model(self):
        # self.prompt = Prompt(prompt_alpha=self.prompt_alpha, image_size=self.image_size,inn_num=self.inn_num).to(self.device)    # image_size=512
        self.prompt = Prompt(prompt_alpha=self.prompt_alpha, image_size=self.image_size,save_results_path= self.save_results_path,num_ViGBlocks=self.num_ViGBlocks).to(self.device)    # image_size=512

        self.model = ResUnet(resnet=self.backbone, num_classes=self.out_ch, pretrained=False, newBN=AdaBN, warm_n=self.warm_n,save_path=self.save_results_path).to(self.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_ViGBlocks',default=1)
    ...
    parser.add_argument('--inn_num', type=int, default=8)
    ...
    # config.Target_Dataset = ['RIM_ONE_r3', 'ORIGA']
    config.Target_Dataset.remove(config.Source_Dataset)


    config.save_results_path = '/data/chengzhicao/VLM/VPTTA-main/OPTIC/visual_feature4/' + 'lr{}'.format(config.lr) + '_memory_size{}'.format(config.memory_size) +"_neighbor{}".format(config.neighbor) + '_optimizer{}'.format(config.optimizer) + '_threshold{}'.format(config.threshold) + '_iters{}'.format(config.iters) +'_inn_num{}'.format(config.inn_num) +"_num_ViGBlocks{}".format(config.num_ViGBlocks) + '_Source_Dataset_{}'.format(config.Source_Dataset) + '_Target_Dataset_{}'.format(config.Target_Dataset[0])
    print("--------------------------------------------------------------")
