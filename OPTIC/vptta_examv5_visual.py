...
if __name__ == '__main__':
    ...
    config.Target_Dataset = ['REFUGE', 'RIM_ONE_r3']
    config.Target_Dataset.remove(config.Source_Dataset)

    config.save_results_path = '/data/chengzhicao/VLM/VPTTA-main/visualization_segmentation_resultsv5_OPTIC_tSNE/OPTICv4_' + 'lr{}'.format(config.lr) + '_memory_size{}'.format(config.memory_size) +"_neighbor{}".format(config.neighbor) + '_optimizer{}'.format(config.optimizer) + '_threshold{}'.format(config.threshold) + '_iters{}'.format(config.iters) + 'Source_Dataset_{}'.format(config.Source_Dataset) + '_Target_Dataset_{}'.format(config.Target_Dataset[0])
    print("--------------------------------------------------------------")
