...
if __name__ == '__main__':
    ...
    #     images_nhwc = images_nhwc.type(torch.float32)
    #     _out = model(images_nhwc)
    #     ORIGA_data.append(_out)
    # ORIGA_data = np.array(ORIGA_data)
    # print('ORIGA_data=',ORIGA_data.shape)

    # path1 = '/data/chengzhicao/VLM/TTDG/datasets/Fundus/REFUGE_Valid/image'
    # REFUGE_Valid_data = []
    # img_list = os.listdir(path1)
    # img_list.sort()
    # for i in range(len(img_list)):
    ...
    #     images_nhwc = images_nhwc.type(torch.float32)
    #     _out = model(images_nhwc)
    #     REFUGE_Valid_data.append(_out)
    # REFUGE_Valid_data = np.array(REFUGE_Valid_data)
    # print('REFUGE_Valid_data=',REFUGE_Valid_data.shape)


    # path1 = '/data/chengzhicao/VLM/TTDG/datasets/Fundus/Drishti_GS/train/image'
    # Drishti_GS_data = []
    # img_list = os.listdir(path1)
    # img_list.sort()
    # for i in range(len(img_list)):
    ...
    #     images_nhwc = images_nhwc.type(torch.float32)
    #     _out = model(images_nhwc)
    #     Drishti_GS_data.append(_out)
    # Drishti_GS_data = np.array(Drishti_GS_data)
    # print('Drishti_GS_data=',Drishti_GS_data.shape)
    ...
    # data3 = ORIGA_data
    # data4 = REFUGE_Valid_data
    # data5 = Drishti_GS_data
    
    # 生成三个类别的样本数据的标签
    label1 = np.zeros(data1.shape[0]) + 0
    label2 = np.zeros(data2.shape[0]) + 1
    # label3 = np.zeros(data3.shape[0]) + 2
    # label4 = np.zeros(data4.shape[0]) + 3
    # label5 = np.zeros(data5.shape[0]) + 4
    ...
    shape_list = ['o', 'o', 'o', 'o','o']  # 设置不同类别的形状
    color_list = ['r', 'g', 'b', 'm','y']  # 设置不同类别的颜色
    label_list = ['RIM_ONE_r3', 'REFUGE', 'ORIGA', 'REFUGE_Valid','Drishti_GS']
    # 遍历所有标签种类
    ...
    # # 遍历所有样本
    color_map = {0:'r', 1:'g', 2:'b', 3:'m',4:'y'}   # 定义类别颜色映射关系
    # shape_map = {0:'o', 1:'D', 2:'^', 3:'P'}
    shape_map = {0:'o',1:'o',2:'o',3:'o',4:'o' }
    # label_map = {0:'Class 1', 1:'Class 2', 2:'Class 3', 3:'Class 4'}
    label_map = {0:'RIM_ONE_r3', 1:'REFUGE',2:'ORIGA',3:'REFUGE_Valid',4:'Drishti_GS'}
    ...
    plt.savefig('/data/chengzhicao/VLM/VPTTA-main/visualization_tSNE_alll_{}.png'.format(RANDOM_STATE))  # 保存图为png格式
    plt.show()
