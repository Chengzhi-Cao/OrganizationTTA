...
def calculate_metrics(test, reference):
    ...
    test = test1.unsqueeze(0)
    reference = reference1.unsqueeze(0)
    ...
    output1 = [dice, enhanced_metric, structure_metric]


    test = test2.unsqueeze(0)
    reference = reference2.unsqueeze(0)
    ...
    output2 = [dice, enhanced_metric, structure_metric]

    output = [[],[],[]]

    # for k in range(len(output1)):
    #     output[0][0] = (output1[0][k] + output2[0][k])/2
    dice_avg = (output1[0][0] + output2[0][0])/2
    enhanced_metric_avg = (output1[1][0] + output2[1][0])/2
    structure_metric_avg = (output1[2][0] + output2[2][0])/2

    output = [[dice_avg],[enhanced_metric_avg],[structure_metric_avg]]
...
def calculate_metrics_save_segment_output(threshold_my,input_img,test, reference,number=0,save_results_path='/data/chengzhicao/VLM/VPTTA-main'):

    _x = input_img[0,:,:,:].cpu().data.numpy() 
    _x = 255*(_x - np.min(_x)) /(np.max(_x)-np.min(_x))
    _x =  np.uint8(_x)
    _x = _x.transpose(1,2,0)
    __x = Image.fromarray(_x)
    img_x_path = '{}_input.jpg'.format(number)
    __x.save(os.path.join(save_results_path,img_x_path))
    ...
    test = test1.unsqueeze(0)
    reference = reference1.unsqueeze(0)
    test, reference = data_process(pred=test, label=reference, threshold=threshold_my)
    _test1_seg = test
    _refernece1 = reference
    enhanced_metric = ea_metric(test, reference)
    structure_metric = sm_metric(test, reference)
    dice = dice_metric(test, reference)
    output1 = [dice, enhanced_metric, structure_metric]
    ...
    test, reference = data_process(pred=test, label=reference, threshold=threshold_my)
    _test2_seg = test
    _refernece2 = reference
    enhanced_metric = ea_metric(test, reference)
    structure_metric = sm_metric(test, reference)
    dice = dice_metric(test, reference)
    output2 = [dice, enhanced_metric, structure_metric]
    # ######################################################################
    # _test_seg = _test1_seg.squeeze()*250 + _test2_seg.squeeze()*125
    # _refernece = _refernece1.squeeze()*250 + _refernece2.squeeze()*125
    # __test_seg = Image.fromarray(_test_seg)
    # img__test_seg_path = '{}_test_seg.jpg'.format(number)
    # __test_seg.save(os.path.join(save_results_path,img__test_seg_path))



    # image = cv.imread(os.path.join(save_results_path,img_x_path))
    # seg_mask = cv.imread(os.path.join(save_results_path,img__test_seg_path))
    
    # (h,w,c) = seg_mask.shape
    # for a in range(h):
    #     for b in range(w):
    #         if seg_mask[a,b,0]> 50 and seg_mask[a,b,0] < 150:
    #             seg_mask[a,b,:] = [0,255,0]
    #         if seg_mask[a,b,0]> 200:
    #             seg_mask[a,b,:] = [0,0,255]
    # image_seg_combine = image + 0.9*seg_mask    
    # seg_img_combine_path = '{}_combine.jpg'.format(number)
    # cv.imwrite(os.path.join(save_results_path,seg_img_combine_path),image_seg_combine)

    # _refernece = Image.fromarray(_refernece)
    # _refernece_path = '{}_reference.jpg'.format(number)
    # _refernece.save(os.path.join(save_results_path,_refernece_path))
    ...
    # image = cv.imread(os.path.join(save_results_path,img_x_path))
    # seg_mask = cv.imread(os.path.join(save_results_path,_refernece_path))
    # (h,w,c) = seg_mask.shape
    # for a in range(h):
    #     for b in range(w):
    #         if seg_mask[a,b,0]> 50 and seg_mask[a,b,0] < 150:
    ...
    #         if seg_mask[a,b,0]> 200:
    #             seg_mask[a,b,:] = [0,0,255]
    # image_seg_combine = image + 0.8*seg_mask    
    # seg_img_combine_path = '{}_reference.jpg'.format(number)
    # cv.imwrite(os.path.join(save_results_path,seg_img_combine_path),image_seg_combine)
