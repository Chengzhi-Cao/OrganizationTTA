...
from utils.prompt_myv6_runningtime import Promptv6 as Prompt
...
if __name__ == '__main__':
    ...
    config.save_results_path = '/data/chengzhicao/VLM/VPTTA-main/OPTIC/visual_feature/' + 'lr{}'.format(config.lr) + '_memory_size{}'.format(config.memory_size) +"_neighbor{}".format(config.neighbor) + '_optimizer{}'.format(config.optimizer) + '_threshold{}'.format(config.threshold) + '_iters{}'.format(config.iters) +'_inn_num{}'.format(config.inn_num) + '_Source_Dataset_{}'.format(config.Source_Dataset) + '_Target_Dataset_{}'.format(config.Target_Dataset[0])
    print("--------------------------------------------------------------")
