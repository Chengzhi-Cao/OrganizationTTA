...
if __name__ == '__main__':
    ...
    model = PraNet(warm_n=config.warm_n).to(config.device)
    checkpoint = torch.load(os.path.join(load_model, 'pretrain-PraNet.pth'))
    model.load_state_dict(checkpoint, strict=True)
    
    
    path1 = '/data/chengzhicao/VLM/TTDG/datasets/Polyp/BKAI/train/image'
    RIM_ONE_r3_data1 = []
    ...
    path1 = '/data/chengzhicao/VLM/VPTTA-main/visualization_segmentation_results_GNN_prompt_POLYPV3/Source_Dataset_BKAI_Target_Dataset_CVC-ClinicDB'
    RIM_ONE_r3_data = []
    img_list = os.listdir(path1)
    img_list.sort()
    for i in range(len(img_list)):
    ...
    path1 = '/data/chengzhicao/VLM/TTDG/datasets/Polyp/CVC-ClinicDB/train/image'
    REFUGE_data = []
    img_list = os.listdir(path1)
    img_list.sort()
    for i in range(len(img_list)):
