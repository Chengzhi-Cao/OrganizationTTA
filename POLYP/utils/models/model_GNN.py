...
class VGNN(nn.Module):
    def __init__(self, in_features=3*16*16, out_feature=320, num_patches=196,
                 num_ViGBlocks=1, num_edges=9, head_num=1):
        super().__init__()
    ...
    def forward(self, x):   # x=[1,3,352,352]
        x = self.patchifier(x)  # x = [1,484,3,16,16]
        B, N, C, H, W = x.shape # B=1, N=484
        x = self.patch_embedding(x.view(B * N, -1)).view(B, N, -1)  # x=[1,484,768]
        x = x + self.pose_embedding # pose_embedding=[1024,768]
