...
from utils.promptv5_visual import Prompt
...
class VPTTA:
    ...
    def build_model(self):
        self.prompt = Prompt(prompt_alpha=self.prompt_alpha, image_size=self.image_size,save_results_path= self.save_results_path).to(self.device)
    ...
    def run(self):
        ...
        for batch, data in enumerate(self.target_test_loader):
            ...
            metrics = calculate_metrics_save_segment_output(threshold_my=self.threshold_my,input_img=x.detach().cpu(),test=seg_output.detach().cpu(), reference=y.detach().cpu(),number=_flag,save_results_path=self.save_results_path)
            # if _flag % 100 == 0:
